...
/**
 * Resolver OpenQASM 3 include sources из in-memory options и явно разрешенных директорий.
 */
public final class OpenQasm3IncludeResolver {
     ...
     */
    public OpenQasm3IncludeResolverResult resolve(final ImportOptions options) {
        if (options == null) {
        ...
        for (int i = 0; i < options.includeDirectoryCount(); i++) {
            ...
            try {
                ...
                return OpenQasm3IncludeResolverResult.failure(IntegrationDiagnostic.error(
                    IntegrationDiagnosticCode.INCLUDE_RESOLUTION_FAILED,
                    "OpenQASM 3 include directory path is invalid: " + options.includeDirectories().get(i) + "."
                ));
            }
            if (!Files.isDirectory(directory)) {
                return OpenQasm3IncludeResolverResult.failure(IntegrationDiagnostic.error(
                    IntegrationDiagnosticCode.INCLUDE_RESOLUTION_FAILED,
                    "OpenQASM 3 include directory does not exist: " + directory + "."
                ));
            }
            try {
        ...
        return OpenQasm3IncludeResolverResult.success(sources);
    }
    ...
    private static void loadDirectorySources(
        ...
        try (DirectoryStream<Path> files = Files.newDirectoryStream(directory)) {
            ...
            throw new IllegalArgumentException("OpenQASM 3 include directory cannot be read.", exception);
        }
    }
    ...
    private static void loadSource(
        ...
        try {
            ...
            throw new IllegalArgumentException("OpenQASM 3 include file cannot be read.", exception);
        }
    }
}
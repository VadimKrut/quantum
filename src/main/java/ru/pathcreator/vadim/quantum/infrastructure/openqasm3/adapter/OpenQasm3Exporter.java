...
import ru.pathcreator.vadim.quantum.infrastructure.openqasm3.normalization.OpenQasm3ExportNormalizationResult;
import ru.pathcreator.vadim.quantum.infrastructure.openqasm3.normalization.OpenQasm3ExportNormalizer;
import ru.pathcreator.vadim.quantum.infrastructure.openqasm3.syntax.OpenQasm3Writer;
import ru.pathcreator.vadim.quantum.infrastructure.openqasm3.syntax.OpenQasm3WriterResult;

/**
 * Exporter Quantum IR в OpenQASM 3.0.
 */
public final class OpenQasm3Exporter implements QuantumExporter {

    /**
     * Writer OpenQASM 3.0.
     */
    private final OpenQasm3Writer writer;
    ...
    /**
     * Adapter-level lowering перед записью OpenQASM 3.
     */
    ...
    /**
     * Создает exporter OpenQASM 3.0.
     */
    public OpenQasm3Exporter() {
        this.writer = new OpenQasm3Writer();
        this.validator = new QuantumProgramValidator();
        this.normalizer = new OpenQasm3ExportNormalizer();
    ...
    public IntegrationCapabilityProfile capabilityProfile() {
        return IntegrationCapabilityProfile.of(
            ...
            EnumSet.of(
                ...
                IntegrationCapability.DYNAMIC_QUBIT_REFERENCES,
    ...
    public ExportResult exportProgram(
        ...
        if (options.textMode() == ExportTextMode.LOSSLESS_WHEN_AVAILABLE) {
            diagnostics.add(IntegrationDiagnostic.warning(
                ...
                "OpenQASM 3 export does not have lossless source text attached to Quantum IR; canonical output will be emitted."
            ));
        ...
        final OpenQasm3ExportNormalizationResult normalizationResult = normalizer.normalize(
            program,
            options
        );
        ...
        final OpenQasm3WriterResult writerResult = writer.write(normalizationResult.program());

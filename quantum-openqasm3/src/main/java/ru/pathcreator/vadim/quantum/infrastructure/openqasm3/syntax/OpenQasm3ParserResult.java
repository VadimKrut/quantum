...
/**
 * Внутренний результат parser OpenQASM 3.
 */
public final class OpenQasm3ParserResult {
     ...
     */
    public OpenQasm3ParserResult(
        final QuantumProgram program,
        final List<IntegrationDiagnostic> diagnostics
    ) {
        if (diagnostics == null) {
            throw new IllegalArgumentException("OpenQASM 3 parser diagnostics must not be null.");
        }
        for (int i = 0; i < diagnostics.size(); i++) {
            if (diagnostics.get(i) == null) {
                throw new IllegalArgumentException("OpenQASM 3 parser diagnostic must not be null.");
            }
        }
    ...
    public QuantumProgram program() {
        if (program == null) {
            throw new IllegalStateException("OpenQASM 3 parser result does not contain a program.");
        }
        return program;
    }
    ...
    public IntegrationDiagnostic diagnostic(final int index) {
        if (
            ...
            throw new IllegalArgumentException("OpenQASM 3 parser diagnostic index is outside of result bounds.");
        }

...
/**
 * Результат adapter-level нормализации Quantum IR перед записью OpenQASM 3.
 */
public final class OpenQasm3ExportNormalizationResult {

    /**
     * Нормализованная программа, готовая для OpenQASM 3 writer.
     */
    ...
    private OpenQasm3ExportNormalizationResult(
     ...
     */
    public static OpenQasm3ExportNormalizationResult success(
        final QuantumProgram program,
        final List<IntegrationDiagnostic> diagnostics
    ) {
        if (program == null) {
        ...
        return new OpenQasm3ExportNormalizationResult(
            program,
     ...
     */
    public static OpenQasm3ExportNormalizationResult failure(final List<IntegrationDiagnostic> diagnostics) {
        return new OpenQasm3ExportNormalizationResult(
            null,
            copyDiagnostics(diagnostics)
        );
    ...
    public QuantumProgram program() {
        if (!isSuccess()) {
            throw new IllegalStateException("OpenQASM 3 normalization did not produce a program.");
        }
        return program;
    }
